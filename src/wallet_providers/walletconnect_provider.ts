...
type EthereumProviderNotifications =
  | "connect"
  | "disconnect"
  | "chainChanged"
  | "accountsChanged"
...
export class WalletConnectEthereumProvider implements EthereumProvider {
  ...
  private async signMessage(args: RequestArguments) {
    ...
    const signature = await (
      this.ethereumProvider as any
    ).connector.signMessage([address.toLowerCase(), utils.keccak256(toSign)]);

    const signer = utils.recoverAddress(
      utils.arrayify(utils.keccak256(toSign)),
      signature
    );
  ...
  private async getUserPendingDeposit(
    assetId: string,
    account: string,
    to: string
  ) {
  ...
  private async approveProof(args: RequestArguments) {
    ...
    return this.sendTransactionAndPoll(args, async () =>
      this.getUserProofApprovalStatus(account, proofHash, to)
    );
  }

  private async getUserProofApprovalStatus(
    account: string,
    proofHash: string,
    to: string
  ) {
  ...
  private async getUserAllowance(
    userAddress: string,
    assetAddress: string,
  ...
  private async sendTransactionAndPoll(
    ...
    try {
      const tx = await Promise.race([
        this.ethereumProvider.request(args),
        confirmFromData,
      ]);
  ...
  private async getLatestTxHash(
    ...
    return (
      ...
      (await this.getLatestTxHash(provider, block.number - 1))
    );
  }
}
